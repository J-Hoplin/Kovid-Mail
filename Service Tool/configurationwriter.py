...
import getpass,yaml,hashlib,sys
from enum import Enum

#from Parent Directory
sys.path.append("..")
from globalutility import GlobalUtilities
...
class ConfigurationWriter(GlobalUtilities):
    ...
    def __init__(self):
        ...
        self.__yamlInstance = self.readConfigYaml(self.__configLocation)
    ...
    def initiatePW(self,forceChange : False):
        if not forceChange:
            if not self.__yamlInstance['configData']['configpw']['value']:
                ...
                self.saveConfigYaml(self.__configLocation,self.__yamlInstance)
                self.__yamlInstance = self.readConfigYaml(self.__configLocation)
        else:
            self.__yamlInstance['configData']['configpw']['value'] = self.encrypt('admin')
            self.saveConfigYaml(self.__configLocation, self.__yamlInstance)
    ...
    def writer(self):
        ...
        for i in features:
            ...
            if writingSector[i]['encrypt']:
                ...
                while loop:
                    try:
                        value = self.hideInput("정보 입력하기 %8s >> " % i)
                        if not value:
                            loop = False
                            forceBreak = True
                        else:
            ...
            #암호화가 필요하지 않은 데이터인 경우
            else:
                loop = True
                while loop:
                    try:
                        value = self.normalInput("정보 입력하기 %8s >> " % i)
                        if not value:
                            loop = False
                            forceBreak = True
                        else:
