...
class GlobalUtilities(object):
    ...
    def saveConfigYaml(self,directory,yamlinstance) -> yaml:
        with open(directory, 'w') as f:
            yaml.dump(yamlinstance,f)

    def returnSelectedOption(self, li,ymlIns = False) -> int:
            ...
            for i in opt:
                if not ymlIns:
                    ...
                    if value['encrypt'] and value['value']:
                        print(f"{i[0]} : {self.decrypt(value['value']).decode('UTF-8')}")
                    else:
